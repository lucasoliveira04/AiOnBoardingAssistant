...
@router.get("/onboarding/project/{project_name}")
def get_project(project_name: ProjectName, db: Session = Depends(get_db)):
    steps = (
        db.query(OnboardingStep)
        .filter(OnboardingStep.project_name == project_name.value)
